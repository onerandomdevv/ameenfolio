...
- `/api/wakatime/status` turns the owner’s server-only WakaTime heartbeat into a cached boolean presence signal and today’s total. Raw heartbeat metadata and the API key never reach the browser.
